...
const initialMessages = [
    { id: 1, sender: 'other', text: 'Hi! Can you quickly review the Mongoose models we discussed?', time: '1:05 PM' },
    { id: 2, sender: 'self', text: 'Sure thing, I just pushed the final Post and User schemas.', time: '1:07 PM' },
    { id: 3, sender: 'other', text: 'Perfect. Let me know when you start the authentication routes.', time: '1:10 PM' },
    { id: 4, sender: 'self', text: 'Will do! Planning to start tomorrow morning. Check the repo then.', time: '1:12 PM' },
];
    ...
    return (
        <div style={{ display: 'flex', justifyContent: isSelf ? 'flex-end' : 'flex-start' }}>
            <div style={{
                ...
                {message.text}
                <div style={{ fontSize: '0.65rem', color: isSelf ? 'rgba(255, 255, 255, 0.7)' : 'var(--gc-color-text-muted)', textAlign: 'right', marginTop: '0.5rem' }}>{message.time}</div>
            </div>
        </div>
    );
};
...
const ChatWindow = ({ activeChatUser }) => {
    ...
    const handleAttach = () => {
        alert("File attachment functionality ready! (MERN backend required to handle upload)");
    };
    ...
    return (
        <div className="gc-card" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 1.5rem', borderBottom: '1px solid var(--gc-color-border)' }}>
                <div>
                    <h3 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--gc-color-heading)' }}>{user.user}</h3>
                    <p style={{ fontSize: '0.8rem', color: 'var(--gc-color-success)' }}>{user.status || 'Active Now'}</p>
                </div>
