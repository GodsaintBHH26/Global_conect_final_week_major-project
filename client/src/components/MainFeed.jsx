...
    });
    
    if (!res.ok) throw new Error("File upload failed on main feed button click.");
    const data = await res.json();
    console.log("Uploaded file info:", data);
    alert(`File uploaded successfully! Path: ${data.filePath}. Now, you must integrate this path into a new post creation.`);
  } catch (err) {
    console.error(err);
...
        <img src={post.userId?.profilePic || user} alt={post.userId?.name || 'User'} style={{ width: "40px", height: "40px", borderRadius: "50%" }} />
        <div>
          <h3 style={{ fontWeight: 600, fontSize: "0.875rem", color: "var(--gc-color-heading)" }}>{post.userId?.name || 'Unknown User'}</h3>
          <p style={{ fontSize: "0.75rem", color: "var(--gc-color-text-muted)" }}>{post.content}</p>
          <p style={{ fontSize: "0.75rem", color: "var(--gc-color-text-muted)" }}>{new Date(post.createdAt).toLocaleString()}</p>
        </div>
      </div>
      <button className="gc-btn-base gc-btn-reset" style={{ color: "var(--gc-color-text-muted)" }}>...</button>
    </div>
    {post.image && <img src={post.image} alt="Post Media" style={{ width: "100%", objectFit: "cover" }} />}
...
      <button className="gc-flex gc-align-center gc-space-x-2 gc-p-2 gc-btn-base gc-btn-reset" style={{ borderRadius: "4px" }}><ThumbsUp size={18} /><span style={{ fontSize: "0.875rem" }}>Like</span></button>
      <button className="gc-flex gc-align-center gc-space-x-2 gc-p-2 gc-btn-base gc-btn-reset" style={{ borderRadius: "4px" }}><MessageSquare size={18} /><span style={{ fontSize: "0.875rem" }}>Comment</span></button>
      <button className="gc-flex gc-align-center gc-space-x-2 gc-p-2 gc-btn-base gc-btn-reset gc-lg-hidden" style={{ borderRadius: "4px" }}><Repeat2 size={18} /><span style={{ fontSize: "0.875rem" }}>Repost</span></button>
      <button className="gc-flex gc-align-center gc-space-x-2 gc-p-2 gc-btn-base gc-btn-reset gc-lg-hidden" style={{ borderRadius: "4px" }}><Send size={18} /><span style={{ fontSize: "0.875rem" }}>Send</span></button>
    </div>
  </div>
);
